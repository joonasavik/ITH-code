...
type <- data_links$Cancer_type[1]
type <- 
print(type)

# data loop
for (type in data_links$Cancer_type){
  time_start <- Sys.time()
  ...
  snv_data$AF_Tumor <- snv_data$t_alt_count / snv_data$t_depth  # what is this?
  snv_data$PN_B <- 0      # NORMAL COPY NUMBER = 2
